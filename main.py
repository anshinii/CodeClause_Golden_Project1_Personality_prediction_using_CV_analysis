# GOLDEN Project Name - Personality_Prediction_CV_Analysis 
#projectee Name - Anshini Kumbhare
...
class train_model:
    ...
    def train(self):
        data =pd.read_csv('E:/CODE CLAUSE/TASK3-GOLDEN_PROJ_PERSONALITY_PREDICTION_USING_CV_ANALYSIS/training_dataset.csv')  # in this line you have to add your Dataset path 
        array = data.values
...
def prediction_result(top, aplcnt_name, cv_path, personality_values):
  ...
  #  result.geometry('700x550')
    ...
    Label(result, text="Result of Personality Prediction by Anshini Kumbhare", foreground='green', bg='white', font=titleFont, pady=10, anchor=CENTER).pack(fill=BOTH)
    ...
    Label(result, text = str('{} : {}'.format("Email:","anshinikumbhare@gmail.com")).title(), foreground='black', bg='white', anchor='w').pack(fill=BOTH)
    Label(result, text = str('{} : {}'.format("Mobile Number:", "8698855995")).title(), foreground='black', bg='white', anchor='w').pack(fill=BOTH)
    Label(result, text = str('{} : {}'.format("Skills:", "C/C++, Python, Machine Learning")).title(), foreground='black', bg='white', anchor='w').pack(fill=BOTH)
    Label(result, text = str('{} : {}'.format("Degree:", "Btech in Computer Science and Engineering")).title(), foreground='black', bg='white', anchor='w').pack(fill=BOTH)
    Label(result, text = str('{} : {}'.format("Experience:","0")).title(), foreground='black', bg='white', anchor='w').pack(fill=BOTH)

    Label(result, text = str('{} : {}'.format("Predicted Personality of Candidate:", personality)), font=titleFont, foreground='red', bg='white', anchor='w').pack(fill=BOTH)
...
if __name__ == "__main__":
    ...
    root.title("Personality Prediction System using CV Analysis")
